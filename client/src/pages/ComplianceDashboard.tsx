...
const STATUS_CONFIG: Record<ComplianceStatus, { icon: React.ElementType; label: string; color: string; bg: string }> = {
  ...
  not_applicable: { icon: AlertTriangle,  label: "Not Applicable",  color: "text-zinc-500",    bg: "bg-zinc-900/40 border-zinc-800/50" },
};
  ...
  return (
    <div className={`rounded-xl border p-5 space-y-4 ${cfg.bg}`}>
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-0.5">
          <div className="flex items-center gap-2">
            <h3 className="text-sm font-semibold text-zinc-100">{j.name}</h3>
            <span className="text-[10px] font-mono text-zinc-500 bg-zinc-800 px-1.5 py-0.5 rounded">
              {j.code}
            </span>
          </div>
          <p className="text-xs text-zinc-500">{j.regulationName}</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <div className="space-y-0.5">
          <span className="text-zinc-600">Battery ID</span>
          <p className="text-zinc-300 font-medium">{j.idFieldName}</p>
        </div>
        <div className="space-y-0.5">
          <span className="text-zinc-600">Mandatory From</span>
          <p className="text-zinc-300 font-medium">
            {j.mandatoryFrom ?? "TBD"}
          </p>
        </div>
        <div className="space-y-0.5">
          <span className="text-zinc-600">Public Passport</span>
          <p className={j.requiresPublicPassport ? "text-emerald-400 font-medium" : "text-zinc-500"}>
            {j.requiresPublicPassport ? "Required" : "Not required"}
          </p>
        </div>
        <div className="space-y-0.5">
          <span className="text-zinc-600">Gov Sync</span>
          <p className={j.requiresGovSync ? "text-amber-400 font-medium" : "text-zinc-500"}>
        ...
        {j.requiresPublicPassport && (
          <Link href={`/passport/${j.passportIdPrefix}/demo`}>
            <Button
              variant="outline"
              size="sm"
              className="h-7 text-xs border-zinc-700 text-zinc-300 hover:text-zinc-100 bg-transparent"
            >
...
function SummaryBar({ activeJurisdictions }: { activeJurisdictions: string[] }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      {[
        { label: "Active Jurisdictions", value: total, color: "text-zinc-100" },
...
function FeatureHighlights() {
  ...
  return (
    <div className="space-y-3">
      <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Compliance Features</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 space-y-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-zinc-100">Compliance Dashboard</h1>
        ...
        <Link href="/settings/platform">
          <Button
            variant="outline"
            size="sm"
            className="h-8 text-xs border-zinc-700 text-zinc-300 hover:text-zinc-100 bg-transparent"
          >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {inactiveList.map((code) => {
              ...
              return (
                <div
                  key={code}
                  className="rounded-xl border border-zinc-800/50 bg-zinc-900/20 p-4 opacity-60 space-y-1"
                >
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-zinc-400">{j.name}</span>
                    <span className="text-[10px] font-mono text-zinc-600 bg-zinc-800 px-1 rounded">{j.code}</span>
                  </div>
                  <p className="text-xs text-zinc-600">{j.regulationName}</p>
                </div>
              );
            })}
          </div>
          <Link href="/settings/platform">
            <Button
              variant="outline"
              size="sm"
              className="h-8 text-xs border-zinc-700 text-zinc-400 hover:text-zinc-100 bg-transparent"
            >
