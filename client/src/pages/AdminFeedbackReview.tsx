...
const FEEDBACK_TYPES = [
  ...
  { value: "general", label: "General", icon: MessageSquare, color: "text-zinc-400", bg: "bg-zinc-500/10 border-zinc-500/20" },
] as const;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Manage wiki article feedback and suggestions</p>
            </div>
          </div>
          <Button
            variant="outline"
        ...
        {stats && (
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-6">
            {[
              { key: "total", label: "Total", value: stats.total, icon: BarChart3, color: "text-zinc-300", bg: "bg-white/5 border-white/10" },
        ...
        <div className="flex flex-wrap items-center gap-3 mb-4">
          ...
          <Select value={statusFilter} onValueChange={(v) => { setStatusFilter(v as StatusType | "all"); setPage(0); }}>
            <SelectTrigger className="w-[140px] bg-white/5 border-white/10 text-zinc-300 h-9">
          ...
          <Select value={typeFilter} onValueChange={(v) => { setTypeFilter(v as FeedbackType | "all"); setPage(0); }}>
            <SelectTrigger className="w-[160px] bg-white/5 border-white/10 text-zinc-300 h-9">
              <Filter className="w-3.5 h-3.5 mr-1.5 text-zinc-500" />
        ...
        {selectedIds.size > 0 && (
          <div className="flex items-center gap-3 mb-4 p-3 bg-primary/5 border border-primary/20 rounded-xl">
            ...
            <div className="flex items-center gap-2 ml-auto">
              ...
              <button onClick={() => setSelectedIds(new Set())} className="text-xs text-zinc-500 hover:text-zinc-300 ml-2">
                Clear
              </button>
            </div>
          </div>
          ...
          <div className={`flex-1 min-w-0 ${selectedItem ? "hidden lg:block" : ""}`}>
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="text-sm font-medium text-zinc-400 mb-1">No feedback found</h3>
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <span className="text-zinc-200 truncate max-w-[200px]">{item.articleTitle}</span>
                            </div>
                            {item.content && (
                              <p className="text-[11px] text-zinc-600 truncate max-w-[250px] mt-0.5 ml-5.5">
                                {item.content}
                              </p>
                            )}
                          </td>
                            ...
                            {item.rating ? (
                              <div className="flex items-center gap-1">
                                {Array.from({ length: 5 }).map((_, i) => (
                                  <Star
                                    key={i}
                                    className={`w-3 h-3 ${i < item.rating! ? "text-amber-400 fill-amber-400" : "text-zinc-700"}`}
                                  />
                                ))}
                              </div>
                            ) : (
              ...
              <div className="bg-white/[0.02] border border-white/5 rounded-xl overflow-hidden sticky top-6">
                ...
                <div className="p-4 border-b border-white/5">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[10px] font-mono text-zinc-600 uppercase">Feedback #{selectedItem.id}</span>
                    <button
                ...
                <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto">
                  {/* User Info */}
                  <div>
                    <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Submitted By</label>
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-lg bg-white/5 flex items-center justify-center">
                        <User className="w-3.5 h-3.5 text-zinc-400" />
                      </div>
                      <div>
                        <p className="text-xs text-zinc-200">{selectedItem.userName || "Anonymous"}</p>
                        {selectedItem.userEmail && (
                          <p className="text-[10px] text-zinc-500">{selectedItem.userEmail}</p>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1">Submitted</label>
                      <div className="flex items-center gap-1.5">
                        <Calendar className="w-3 h-3 text-zinc-600" />
                        <span className="text-xs text-zinc-400">{formatDateTime(selectedItem.createdAt)}</span>
                      </div>
                    </div>
                    {selectedItem.reviewedAt && (
                      <div>
                        <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1">Reviewed</label>
                        <div className="flex items-center gap-1.5">
                          <Calendar className="w-3 h-3 text-zinc-600" />
                          <span className="text-xs text-zinc-400">{formatDateTime(selectedItem.reviewedAt)}</span>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  {selectedItem.rating && (
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Rating</label>
                      <div className="flex items-center gap-1">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star
                            key={i}
                            className={`w-4 h-4 ${i < selectedItem.rating! ? "text-amber-400 fill-amber-400" : "text-zinc-700"}`}
                          />
                        ))}
                        <span className="text-xs text-zinc-400 ml-2">{selectedItem.rating}/5</span>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedItem.section && (
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Article Section</label>
                      <p className="text-xs text-zinc-300 bg-white/5 rounded-lg px-3 py-2">{selectedItem.section}</p>
                    </div>
                  )}
                  ...
                  {selectedItem.content && (
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Feedback Content</label>
                      <div className="text-xs text-zinc-300 bg-white/5 rounded-lg px-3 py-2 leading-relaxed whitespace-pre-wrap max-h-40 overflow-y-auto">
                        {selectedItem.content}
                      </div>
                    </div>
                  )}
                  ...
                  {selectedItem.suggestedContent && (
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Suggested Edit</label>
                  ...
                  {selectedItem.reviewNotes && (
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Review Notes</label>
                      <div className="text-xs text-zinc-300 bg-violet-500/5 border border-violet-500/10 rounded-lg px-3 py-2 leading-relaxed">
                        {selectedItem.reviewNotes}
                      </div>
                    </div>
                  )}
                ...
                {selectedItem.status === "pending" && (
                  <div className="p-4 border-t border-white/5 space-y-3">
                    <div>
                      <label className="text-[10px] text-zinc-600 uppercase tracking-wider block mb-1.5">Review Notes (optional)</label>
                      <div className="flex gap-2">
                        <Input
                          placeholder="Add review notes..."
                          value={reviewNotes}
                          onChange={(e) => setReviewNotes(e.target.value)}
                          className="bg-white/5 border-white/10 text-white placeholder:text-zinc-600 h-8 text-xs"
                        />
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
